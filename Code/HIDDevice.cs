...
namespace PACEBuzz
{
    ...
    public class HIDDeviceException : ApplicationException
    {
        ...
        public static HIDDeviceException GenerateWithWinError(string strMessage)
        {
            ...
            return new HIDDeviceException(string.Format("Msg:{0} WinEr:{1}", strMessage, lastError));
        }
    }
    ...
    public abstract class HIDDevice : Win32Usb, IDisposable
    {
        ...
        protected virtual void Dispose(bool bDisposing)
        {
            ...
            {
                // This will sometimes fail because we're trying to close a handle which has already been deleted
                Console.WriteLine(e);
            }
        }
        ...
        private void Initialise(string strPath)
        {
            ...
            // This method works on Windows 10 x64
            // TODO: Verify that it works on other versions of Windows
            ...
            // m_hHandle = CreateFile(strPath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, IntPtr.Zero, OPEN_EXISTING, FILE_FLAG_DELETE_ON_CLOSE, IntPtr.Zero);
            //m_hHandle = CreateFile(strPath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, IntPtr.Zero, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, IntPtr.Zero);
        ...
        private void BeginAsyncRead()
        {
            ...
            // put the buffer we used to receive the stuff as the async state then we can get at it when the read completes
        ...
        /// <summary>
        /// Finds all devices that match a PID and VID
        /// </summary>
